...
.then(datos=>{
    ...
    socket.on("historial", mensajes=>{
        mensajes.forEach(m=>{
            divMensajes.innerHTML+=`<br><strong>${m.nombre}</strong> dice: <i>${m.mensaje}</i><br>`
        })
    })
    ...
    socket.on("nuevoMensaje", (nombre, mensaje)=>{
        divMensajes.innerHTML+=`<br><strong>${nombre}</strong> dice: <i>${mensaje}</i><br>`
    })

    socket.on("saleUsuario", nombre=>{
        divMensajes.innerHTML+=`<br><strong>${nombre}</strong> ha salido del chat :( <br>`
    })
